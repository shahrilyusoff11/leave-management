...
func (ls *LeaveService) ProcessWorkflowAction(requestID, actorID uuid.UUID, action models.WorkflowStepAction, comment string) (*models.LeaveRequestWorkflowState, error) {
	...
	if err := ls.db.Preload("CurrentStep").Where("leave_request_id = ?", requestID).First(&workflowState).Error; err != nil {
		return nil, fmt.Errorf("active workflow not found for request: %w", err)
	}

	if workflowState.CurrentStep == nil {
		return nil, errors.New("workflow state exists but has no current step")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to determine responsible users: %w", err)
	}
	...
	if !isAuthorized {
		return nil, fmt.Errorf("user %s is not authorized to approve this step (requires: %s)",
	...
	err = ls.db.Transaction(func(tx *gorm.DB) error {
		var err error
		updatedState, err = ls.workflowSvc.ProcessActionWithTx(tx, requestID, action, actorID, comment)
		if err != nil {
			return err
		}
...
func (ls *LeaveService) ConvertLeaveType(requestID, actorID uuid.UUID, newType models.LeaveType, reason string) error {
	var request models.LeaveRequest
	...
	err := ls.db.Transaction(func(tx *gorm.DB) error {
		...
		if request.Status != models.StatusPending && request.Status != models.StatusEscalated {
			return errors.New("only pending leave requests can be converted")
		}
		...
		var state models.LeaveRequestWorkflowState
		if err := tx.Where("leave_request_id = ?", requestID).First(&state).Error; err == nil {
			state.IsComplete = true
			state.FinalStatus = models.StatusApproved
			state.CurrentStepID = nil
			state.CompletedAt = &now
			state.ActionBy = &actorID
			state.ActionComment = reason
			state.ActionTaken = getConversionAction(newType)
			state.UpdatedAt = now
	...
	if ls.notificationSvc != nil {
		msg := fmt.Sprintf("Your leave request has been converted to %s and approved.", request.LeaveType)
		_ = ls.notificationSvc.SendNotification(request.UserID, "Leave Request Update", msg, models.NotificationTypeLeave, &request.ID)
	}
...
func getConversionAction(newType models.LeaveType) models.WorkflowStepAction {
	switch newType {
	case models.LeaveTypeUnpaid:
		return models.StepActionConvertedToUL
	case models.LeaveTypeEmergency:
